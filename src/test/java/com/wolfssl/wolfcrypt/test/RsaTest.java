...
public class RsaTest {
    ...
    public void testDirectRejectsBadInput() throws Exception {
        ...
        /* wolfSSL requires 1 < input < n - 1 for private decrypt */
        BigInteger n = ((RSAPublicKey)KeyFactory.getInstance("RSA")
            .generatePublic(new X509EncodedKeySpec(key.exportPublicDer())))
            .getModulus();

        byte[] zero = new byte[keySz];
        byte[] one = new byte[keySz];
        one[keySz - 1] = 1;
        byte[] nMinusOne =
            Util.toFixedLength(n.subtract(BigInteger.ONE), keySz);
        byte[] nMinusTwo =
            Util.toFixedLength(n.subtract(BigInteger.valueOf(2)), keySz);

        byte[][] outOfRange = { zero, one, nMinusOne, big };

        for (byte[] in : outOfRange) {
            try {
                key.direct(in, Rsa.RSA_PRIVATE_DECRYPT, rng);
                fail("out of range input should throw");
            } catch (WolfCryptException e) {
                assertEquals(WolfCryptError.RSA_OUT_OF_RANGE_E, e.getError());
            }
        }
        /* n - 2 is the largest accepted value */
        assertEquals(keySz,
            key.direct(nMinusTwo, Rsa.RSA_PRIVATE_DECRYPT, rng).length);
