...
public class WolfCryptCipherTest {
    ...
    public void testGCMInternalIv() throws Exception {
        ...
        /* the internal IV is used once, another encrypt needs a new init */
        try {
            enc.doFinal(pt);
            fail("Second doFinal() with the same key and IV should throw");
        } catch (IllegalStateException e) {
            /* expected */
        }
        try {
            enc.update(pt);
            fail("update() after doFinal() with the same IV should throw");
        } catch (IllegalStateException e) {
            /* expected */
        }
    ...
    public void testGCMInternalIvInterop() throws Exception {
        ...
        if (!enabledJCEAlgos.contains("AES/GCM/NoPadding") ||
            !FeatureDetect.Aes256Enabled() || interopProvider == null) {
            return;
        }
