...
public class WolfCryptCipherTest {
    ...
    /*
     * Test that many small update() calls match a single call. update() may
     * return ciphertext (streaming GCM) or buffer it until doFinal(), so
     * compare the concatenation of all output.
     */
    @Test
    public void testAesGcmChunkedUpdateMatchesSingle() throws Exception {
        ...
        ByteArrayOutputStream chunkedOut = new ByteArrayOutputStream();
        for (int i = 0; i < plaintext.length; i++) {
            byte[] out = chunked.update(plaintext, i, 1);
            if (out != null) {
                chunkedOut.write(out);
            }
        }
        chunkedOut.write(chunked.doFinal());

        assertArrayEquals("Chunked update should match single doFinal",
            expected, chunkedOut.toByteArray());
    }
    ...
    public void testAesGcmLargeThenSmallReusesCipher() throws Exception {
        ...
        for (int i = 0; i < large.length; i += 4096) {
            byte[] out = c.update(large, i, Math.min(4096, large.length - i));
            if (out != null) {
                largeOut.write(out);
            }
        }
        largeOut.write(c.doFinal());
        byte[] largeCipher = largeOut.toByteArray();
