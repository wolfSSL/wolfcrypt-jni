...
/**
 * JUnit4 tests for WolfCryptEdDSAKeyFactory (Ed25519).
 */
public class WolfCryptEdDSAKeyFactoryTest {
    ...
    public static void testProviderInstallation() {
        ...
        System.out.println("JCE WolfCryptEdDSAKeyFactory Class");
    }

    /** Generate a fresh Ed25519 key pair via wolfJCE. */
    private static KeyPair generateKeyPair() throws Exception {
        KeyPairGenerator kpg = KeyPairGenerator.getInstance("Ed25519", "wolfJCE");
        return kpg.generateKeyPair();
    }

    /** Get the Ed25519 KeyFactory from wolfJCE. */
    private static KeyFactory keyFactory() throws Exception {
        return KeyFactory.getInstance("Ed25519", "wolfJCE");
    }
    ...
    public void testPrivateKeyPKCS8RoundTrip() throws Exception {
        ...
        assertEquals("Ed25519 PKCS#8 must be 48 bytes", 48, pkcs8.length);
    ...
    public void testPublicKeySpkiRoundTrip() throws Exception {
        ...
        assertEquals("Ed25519 SPKI must be 44 bytes", 44, spki.length);
    ...
    /**
     * Private key EdECPrivateKeySpec round-trip: generate → getKeySpec →
     * generatePrivate → encodings match.
     */
    @Test
    public void testPrivateKeyEdECSpecRoundTrip() throws Exception {
        Assume.assumeTrue(FeatureDetect.Ed25519Enabled());
        ...
        KeyFactory kf = keyFactory();
        EdECPrivateKeySpec spec = kf.getKeySpec(priv, EdECPrivateKeySpec.class);
        assertNotNull("EdECPrivateKeySpec must not be null", spec);
        assertEquals("Spec params must be Ed25519",
            "Ed25519", spec.getParams().getName());

        PrivateKey reconstructed = kf.generatePrivate(spec);
        assertNotNull("Reconstructed private key must not be null", reconstructed);
        assertArrayEquals("EdECPrivateKeySpec round-trip encoding mismatch",
            originalEncoded, reconstructed.getEncoded());
    }

    /**
     * Public key EdECPublicKeySpec round-trip: generate → getKeySpec →
     * generatePublic → encodings match.
     */
    @Test
    public void testPublicKeyEdECSpecRoundTrip() throws Exception {
        Assume.assumeTrue(FeatureDetect.Ed25519Enabled());
        ...
        KeyFactory kf = keyFactory();
        EdECPublicKeySpec spec = kf.getKeySpec(pub, EdECPublicKeySpec.class);
        assertNotNull("EdECPublicKeySpec must not be null", spec);
        assertEquals("Spec params must be Ed25519",
            "Ed25519", spec.getParams().getName());

        PublicKey reconstructed = kf.generatePublic(spec);
        assertNotNull("Reconstructed public key must not be null", reconstructed);
        assertArrayEquals("EdECPublicKeySpec round-trip encoding mismatch",
            originalEncoded, reconstructed.getEncoded());
    }
    ...
    /**
     * EdDSA alias for the KeyFactory resolves to the same implementation.
     */
    @Test
    public void testEdDSAKeyFactoryAlias() throws Exception {
        Assume.assumeTrue(FeatureDetect.Ed25519Enabled());
        ...
        KeyFactory kf = KeyFactory.getInstance("EdDSA", "wolfJCE");
        PublicKey reconstructed = kf.generatePublic(new X509EncodedKeySpec(spki));
        assertArrayEquals("EdDSA alias KeyFactory SPKI mismatch",
            spki, reconstructed.getEncoded());
    }
}
