        ...
        switch (this.keyType) {
            ...
            case WC_RSA: {
                ...
                try {
                    ...
                    pCopy = Arrays.copyOf(p, (int)pSz[0]);
                    qCopy = Arrays.copyOf(q, (int)qSz[0]);
                    biN = new BigInteger(1, Arrays.copyOf(n, (int)nSz[0]));
                    biP = new BigInteger(1, pCopy);
                    biQ = new BigInteger(1, qCopy);
                    validated = true;
                } catch (WolfCryptException wce) {
                    /* raw key export not compiled in; skip consistency check */
                } finally {
                    /* Zero CRT components regardless of outcome. biP/biQ
                     * are immutable and cannot be wiped. */
                    zeroArray(d);
                    zeroArray(p);
                    zeroArray(q);
                    zeroArray(pCopy);
