...
public class Sha512 extends NativeStruct {
	...
	private native void sha512Update( ByteBuffer data, int position, int len);
	private native void sha512Update( byte[] data, int offset, int len);
	private native void sha512Final( ByteBuffer hash, int position);
    ...
    public void update(ByteBuffer data, int len)
        ...
        if (state == WolfCryptState.INITIALIZED) {
            ...
            sha512Update(data, data.position(), len);
    ...
    public void digest(ByteBuffer hash)
        ...
        if (state == WolfCryptState.INITIALIZED) {
            ...
            if ((hash.remaining() - hash.position()) < Sha512.DIGEST_SIZE)
                throw new ShortBufferException(
                    "Input buffer is too small for digest size");

            sha512Final(hash, hash.position());

            hash.position(hash.position() + Sha512.DIGEST_SIZE);
    ...
    public void digest(byte[] hash)
        ...
        if (state == WolfCryptState.INITIALIZED) {
            ...
            if (hash.length < Sha512.DIGEST_SIZE)
                throw new ShortBufferException(
                    "Input buffer is too small for digest size");
